...
- **🧠 間隔重複系統 (Spaced Repetition System, SRS)**：
  ...
  - **三種學習模式**：
    ...
    - **🧠 SRS 模式 (Review)**：專注於今日到期任務，答題後需**手動評分** (重來/困難/良好/簡單)。
    - **⚡ 快速測驗 (Quiz)**：針對測驗題的最佳化模式。同樣處理今日任務，但採用**自動評分**機制，追求極速刷題體驗。
- **📊 多樣化題型支援**：
...
4.  **選擇模式**：
    *   **SRS 模式**：傳統翻卡學習，適合深度記憶。
    *   **快速測驗**：適合選擇題，系統會根據您的答錯次數自動評分（一次對=良好，錯一次=困難，錯兩次=重來）。
