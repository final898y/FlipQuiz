...
- **間隔重複系統 (Spaced Repetition System, SRS)**：
  ...
  - **四種學習模式**：
    ...
    - **自動 SRS (Auto Review)**：**選擇題專用模式**。系統自動過濾非選擇題，並依答錯次數自動評分與計算間隔。
    - **快速測驗 (Exam Mode)**：**選擇題專用模式**。提供即時記分板與結算報告，完全不影響 SRS 排程。
- **多樣化題型支援**：
  - **翻卡模式 (Flashcard)**：經典正反面記憶卡。
  - **測驗模式 (Quiz)**：互動式選擇題，提供即時對錯回饋。
- **UI/UX 設計 (New)**：
  - **動態模式提示**：切換模式時自動顯示該模式的適用題型與規則說明，提升操作明確性。
  - **語音合成 (TTS)**：支援 Web Speech API。可在卡片正反面點擊「喇叭圖示」或使用快捷鍵朗讀內容。
...
| `answer`          | **是** | 背面顯示的文字 (正確答案)             | 蘋果           | 必須與 `options` 中的正確選項文字完全一致，否則程式比對會失敗 |
...
| `lang_front`      |   否   | 正面朗讀語言                          | en-US          | 預設 `en-US`                                                  |
| `lang_back`       |   否   | 背面朗讀語言                          | zh-TW          | 預設 `zh-TW`                                                  |
